...
public class SearchFragment extends Fragment {
    ...
    public View onCreateView(LayoutInflater inflater, ViewGroup container, Bundle savedInstanceState) {
        ...
        for(int ni = 0; ni < 10;ni++){
            ...
            try {
                JSONObject j = new JSONObject("{\"id\":12,\"sale_id\":200,\"name\":\"BEANS McGEE\",\"activity\":\"jumpin your moms bones\"}");
                _r.hydrate(j,true);
                results.add(_r);
            } catch(Exception e){}
        }
        SearchAdapter s = new SearchAdapter(results);
        ((ListView) v.findViewById(R.id.listview)).setAdapter(s);
        return v;
    }
    ...
    @Subscribe public void onNavStateChange(NavStatus ns){
        ...
        if(ns.state == NavStatus.State.SEARCHING && t != null){
            ...
            try {
                ...
                api.request(Request.Method.GET, "ticket/search/test", params, new Response.Listener<JSONObject>() {
                    @Override
                    public void onResponse(JSONObject ret) {
                        try {
                            ...
                            ((BaseAdapter)((ListView) getView().findViewById(R.id.listview)).getAdapter()).notifyDataSetChanged();
                        } catch(Exception e){}
                    }
